...
1. **Scroll container** — ближайший предок с прокруткой, к которому применяются правила snapping.
2. **Snap positions действуют только на этот контейнер**, а не на все вложенные уровни сразу. Если у элемента есть несколько scroll-предков, его snap-точки влияют только на ближайший подходящий контейнер.
3. **Snapport** — не просто видимая область, а именно та часть scrollport, которую браузер использует для вычисления выравнивания. Обычно она совпадает с видимой зоной, но `scroll-padding` может намеренно её сузить или сдвинуть.
4. **Snap area** строится из рамки элемента. Спецификация говорит о границах, полученных после трансформаций, а `scroll-margin` потом расширяет эту область для расчёта привязки.
5. **Snap position** появляется в тот момент, когда выбранная сторона или центр `snap area` совпадают с нужной точкой `snapport`. Конкретно эту связь задаёт `scroll-snap-align`.
