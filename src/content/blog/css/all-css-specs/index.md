---
title: "Все CSS-спецификации сгруппированные по темам"
description: "Решил я тут себе план для статей создать. Пройдусь по всем спекам и уйду на покой"
pubDate: "Mar 11 2026"
---

Все-все-все CSS-спецификации со страницы https://www.w3.org/TR/?filter-tr-name=&tags%5B%5D=css сгруппированные по темам.

Пожалуй пойду по группам. Это будет моим планом статей на ближайшие 10 лет =)

### 1. Скроллинг и переполнение (Scroll & Overflow)
- CSS Overflow Module [Level 3](https://www.w3.org/TR/css-overflow-3/), [Level 4](https://www.w3.org/TR/css-overflow-4/), [Level 5](https://www.w3.org/TR/css-overflow-5/)
- ✅ CSS Scroll Snap Module [Level 1](https://www.w3.org/TR/css-scroll-snap-1/), [Level 2](https://www.w3.org/TR/css-scroll-snap-2/) ([Статья в блоге](/blog/css/scroll-snap/))
- [Scroll-driven Animations](https://www.w3.org/TR/scroll-animations-1/)
- ✅ [CSS Scroll Anchoring Module Level 1](https://www.w3.org/TR/css-scroll-anchoring-1/) ([Статья в блоге](/blog/css/scroll-anchoring/))
- ✅ [CSS Overscroll Behavior Module Level 1](https://www.w3.org/TR/css-overscroll-1/) ([Статья в блоге](/blog/css/overscroll-behavior/))
- ✅ [CSS Scrollbars Styling Module Level 1](https://www.w3.org/TR/css-scrollbars-1/) ([Статья в блоге](/blog/css/scrollbars-styling/))

### 2. Макеты и позиционирование (Layout & Positioning)
- CSS Grid Layout Module [Level 1](https://www.w3.org/TR/css-grid-1/), [Level 2](https://www.w3.org/TR/css-grid-2/), [Level 3](https://www.w3.org/TR/css-grid-3/)
- [CSS Flexible Box Layout Module Level 1](https://www.w3.org/TR/css-flexbox-1/)
- CSS Multi-column Layout Module [Level 1](https://www.w3.org/TR/css-multicol-1/), [Level 2](https://www.w3.org/TR/css-multicol-2/)
- CSS Positioned Layout Module [Level 3](https://www.w3.org/TR/css-position-3/), [Level 4](https://www.w3.org/TR/css-position-4/)
- CSS Display Module [Level 3](https://www.w3.org/TR/css-display-3/), [Level 4](https://www.w3.org/TR/css-display-4/)
- [CSS Box Alignment Module Level 3](https://www.w3.org/TR/css-align-3/)
- CSS Box Model Module [Level 3](https://www.w3.org/TR/css-box-3/), [Level 4](https://www.w3.org/TR/css-box-4/)
- CSS Box Sizing Module [Level 3](https://www.w3.org/TR/css-sizing-3/), [Level 4](https://www.w3.org/TR/css-sizing-4/)
- CSS Writing Modes [Level 3](https://www.w3.org/TR/css-writing-modes-3/), [Level 4](https://www.w3.org/TR/css-writing-modes-4/)
- [CSS Logical Properties and Values Module Level 1](https://www.w3.org/TR/css-logical-1/)
- [CSS Regions Module Level 1](https://www.w3.org/TR/css-regions-1/)
- [CSS Exclusions Module Level 1](https://www.w3.org/TR/css3-exclusions/)
- [CSS Shapes Module Level 1](https://www.w3.org/TR/css-shapes-1/)
- [CSS Template Layout Module](https://www.w3.org/TR/css-template-3/)
- [CSS Page Floats](https://www.w3.org/TR/css-page-floats-3/)
- [CSS Line Grid Module Level 1](https://www.w3.org/TR/css-line-grid-1/)
- [CSS Round Display Level 1](https://www.w3.org/TR/css-round-display-1/)

### 3. Типографика, шрифты и текст (Typography, Fonts & Text)
- CSS Fonts Module [Level 3](https://www.w3.org/TR/css-fonts-3/), [Level 4](https://www.w3.org/TR/css-fonts-4/), [Level 5](https://www.w3.org/TR/css-fonts-5/)
- [CSS Font Loading Module Level 3](https://www.w3.org/TR/css-font-loading-3/)
- CSS Text Module [Level 3](https://www.w3.org/TR/css-text-3/), [Level 4](https://www.w3.org/TR/css-text-4/)
- CSS Text Decoration Module [Level 3](https://www.w3.org/TR/css-text-decor-3/), [Level 4](https://www.w3.org/TR/css-text-decor-4/)
- [CSS Ruby Annotation Layout Module Level 1](https://www.w3.org/TR/css-ruby-1/)
- CSS Writing Modes [Level 3](https://www.w3.org/TR/css-writing-modes-3/), [Level 4](https://www.w3.org/TR/css-writing-modes-4/) (также влияют на направление текста)
- [CSS Rhythmic Sizing Module Level 1](https://www.w3.org/TR/css-rhythm-1/)
- [Requirements for Chinese Text Layout (中文排版需求)](https://www.w3.org/TR/clreq/)
- [Requirements for Japanese Text Layout (日本語組版処理の要件)](https://www.w3.org/TR/jlreq/)
- [Requirements for Hangul Text Layout and Typography](https://www.w3.org/TR/klreq/)
- [Ethiopic Layout Requirements](https://www.w3.org/TR/elreq/)
- [Ready-made Counter Styles](https://www.w3.org/TR/predefined-counter-styles/)

### 4. Цвет, фон, границы и визуальные эффекты (Color, Backgrounds, Borders & Visual Effects)
- CSS Color Module [Level 3](https://www.w3.org/TR/css-color-3/), [Level 4](https://www.w3.org/TR/css-color-4/), [Level 5](https://www.w3.org/TR/css-color-5/)
- [CSS Color HDR Module Level 1](https://www.w3.org/TR/css-color-hdr-1/)
- [CSS Color Adjustment Module Level 1](https://www.w3.org/TR/css-color-adjust-1/)
- [CSS Backgrounds and Borders Module Level 3](https://www.w3.org/TR/css-backgrounds-3/)
- [CSS Backgrounds Module Level 4](https://www.w3.org/TR/css-backgrounds-4/)
- [CSS Borders and Box Decorations Module Level 4](https://www.w3.org/TR/css-borders-4/)
- ✅ [CSS Gap Decorations Module Level 1](https://www.w3.org/TR/css-gaps-1/)
- [Compositing and Blending Level 1](https://www.w3.org/TR/compositing-1/)
- [Filter Effects Module Level 1](https://www.w3.org/TR/filter-effects-1/)
- [CSS Fill and Stroke Module Level 3](https://www.w3.org/TR/fill-stroke-3/)
- [CSS Masking Module Level 1](https://www.w3.org/TR/css-masking-1/)
- [CSS Shadow Parts Module Level 1](https://www.w3.org/TR/css-shadow-parts-1/)

### 5. Анимация, переходы и трансформации (Animations, Transitions & Transforms)
- CSS Animations [Level 1](https://www.w3.org/TR/css-animations-1/), [Level 2](https://www.w3.org/TR/css-animations-2/)
- CSS Transitions Module [Level 1](https://www.w3.org/TR/css-transitions-1/), [Level 2](https://www.w3.org/TR/css-transitions-2/)
- [Web Animations](https://www.w3.org/TR/web-animations-1/)
- [Web Animations Module Level 2](https://www.w3.org/TR/web-animations-2/)
- [CSS Animation Worklet API](https://www.w3.org/TR/css-animation-worklet-1/)
- CSS View Transitions Module [Level 1](https://www.w3.org/TR/css-view-transitions-1/), [Level 2](https://www.w3.org/TR/css-view-transitions-2/)
- CSS Transforms Module [Level 1](https://www.w3.org/TR/css-transforms-1/), [Level 2](https://www.w3.org/TR/css-transforms-2/)
- [Motion Path Module Level 1](https://www.w3.org/TR/motion-1/)
- CSS Easing Functions [Level 1](https://www.w3.org/TR/css-easing-1/), [Level 2](https://www.w3.org/TR/css-easing-2/)

### 6. Пользовательский интерфейс и взаимодействие (User Interface & Interaction)
- [CSS Basic User Interface Module Level 3 (CSS3 UI)](https://www.w3.org/TR/css-ui-3/), [CSS Basic User Interface Module Level 4](https://www.w3.org/TR/css-ui-4/)
- [CSS Form Control Styling Level 1](https://www.w3.org/TR/css-forms-1/)
- [CSS Spatial Navigation Level 1](https://www.w3.org/TR/css-nav-1/)
- [CSS Will Change Module Level 1](https://www.w3.org/TR/css-will-change-1/)
- [Resize Observer](https://www.w3.org/TR/resize-observer-1/)
- [CSS Environment Variables Module Level 1](https://www.w3.org/TR/css-env-1/)
- [CSS Custom Highlight API Module Level 1](https://www.w3.org/TR/css-highlight-api-1/)

### 7. Медиа-запросы и условные правила (Media Queries & Conditional Rules)
- Media Queries [Level 3](https://www.w3.org/TR/mediaqueries-3/), [Level 4](https://www.w3.org/TR/mediaqueries-4/), [Level 5](https://www.w3.org/TR/mediaqueries-5/)
- CSS Conditional Rules Module [Level 3](https://www.w3.org/TR/css-conditional-3/), [Level 4](https://www.w3.org/TR/css-conditional-4/), [Level 5](https://www.w3.org/TR/css-conditional-5/)

### 8. Селекторы и псевдоэлементы (Selectors & Pseudo-elements)
- Selectors [Level 3](https://www.w3.org/TR/selectors-3/), [Level 4](https://www.w3.org/TR/selectors-4/), [Level 5](https://www.w3.org/TR/selectors-5/)
- [CSS Pseudo-Elements Module Level 4](https://www.w3.org/TR/css-pseudo-4/)

### 9. Генерируемый контент, списки и счётчики (Generated Content, Lists & Counters)
- [CSS Generated Content Module Level 3](https://www.w3.org/TR/css-content-3/)
- [CSS Generated Content for Paged Media Module](https://www.w3.org/TR/css-gcpm-3/)
- [CSS Lists and Counters Module Level 3](https://www.w3.org/TR/css-lists-3/)
- [CSS Counter Styles Level 3](https://www.w3.org/TR/css-counter-styles-3/)

### 10. Печатные и постраничные форматы (Paged Media & Print)
- [CSS Paged Media Module Level 3](https://www.w3.org/TR/css-page-3/)
- [CSS Generated Content for Paged Media Module](https://www.w3.org/TR/css-gcpm-3/)
- [CSS Page Floats](https://www.w3.org/TR/css-page-floats-3/)
- CSS Fragmentation Module [Level 3](https://www.w3.org/TR/css-break-3/), [Level 4](https://www.w3.org/TR/css-break-4/)

### 11. Значения, единицы, каскад и наследование (Values, Units, Cascade & Inheritance)
- CSS Values and Units Module [Level 3](https://www.w3.org/TR/css-values-3/), [Level 4](https://www.w3.org/TR/css-values-4/), [Level 5](https://www.w3.org/TR/css-values-5/)
- CSS Cascading and Inheritance [Level 3](https://www.w3.org/TR/css-cascade-3/), [Level 4](https://www.w3.org/TR/css-cascade-4/), [Level 5](https://www.w3.org/TR/css-cascade-5/), [Level 6](https://www.w3.org/TR/css-cascade-6/)
- [CSS Custom Properties for Cascading Variables Module Level 1](https://www.w3.org/TR/css-variables-1/)
- [CSS Properties and Values API Level 1](https://www.w3.org/TR/css-properties-values-api-1/)

### 12. Синтаксис, парсинг и организация кода (Syntax, Parsing & Code Organization)
- [CSS Syntax Module Level 3](https://www.w3.org/TR/css-syntax-3/)
- [CSS Nesting Module Level 1](https://www.w3.org/TR/css-nesting-1/)
- ✅ [CSS Functions and Mixins Module](https://www.w3.org/TR/css-mixins-1/)
- [CSS Namespaces Module Level 3](https://www.w3.org/TR/css-namespaces-3/)
- [CSS Style Attributes](https://www.w3.org/TR/css-style-attr/)
- [CSS Scoping Module Level 1](https://www.w3.org/TR/css-scoping-1/)
- [CSS Shadow Parts Module Level 1](https://www.w3.org/TR/css-shadow-parts-1/) (также относится к теневому DOM)

### 13. CSS Object Model (CSSOM) и API
- [CSS Object Model (CSSOM)](https://www.w3.org/TR/cssom-1/)
- [CSSOM View Module](https://www.w3.org/TR/cssom-view-1/)
- [CSS Typed OM Level 1](https://www.w3.org/TR/css-typed-om-1/)
- [CSS Painting API Level 1](https://www.w3.org/TR/css-paint-api-1/)
- [CSS Layout API Level 1](https://www.w3.org/TR/css-layout-api-1/)
- [Geometry Interfaces Module Level 1](https://www.w3.org/TR/geometry-1/)
- [CSS Properties and Values API Level 1](https://www.w3.org/TR/css-properties-values-api-1/) (дублируется, но уместно)
- [CSS Animation Worklet API](https://www.w3.org/TR/css-animation-worklet-1/)
- [CSS Custom Highlight API Module Level 1](https://www.w3.org/TR/css-highlight-api-1/)

### 14. Таблицы (Tables)
- [CSS Table Module Level 3](https://www.w3.org/TR/css-tables-3/)

### 15. Речь и синтез (Speech)
- [CSS Speech Module Level 1](https://www.w3.org/TR/css-speech-1/)

### 16. Viewport (область просмотра)
- [CSS Viewport Module Level 1](https://www.w3.org/TR/css-viewport-1/)

### 17. Субтитры и временные текстовые форматы (Timed Text / Subtitles)
- [Timed Text Markup Language 2 (TTML2)](https://www.w3.org/TR/ttml2/)
- [TTML Media Type Definition and Profile Registry](https://www.w3.org/TR/ttml-profile-registry/)
- [Dubbing and Audio description Profiles of TTML2](https://www.w3.org/TR/dapt/)
- [WebVTT: The Web Video Text Tracks Format](https://www.w3.org/TR/webvtt1/)

### 18. Спецификации интеграции (Integration with other standards)
- [SVG Integration](https://www.w3.org/TR/svg-integration/)
- [A MathML for CSS Profile](https://www.w3.org/TR/mathml-for-css/)
- [Associating Style Sheets with XML documents 1.0](https://www.w3.org/TR/xml-stylesheet/)

### 19. Доступность и международные требования (Accessibility & Internationalization)
- [CSS Techniques for Web Content Accessibility Guidelines 1.0](https://www.w3.org/TR/WCAG10-CSS-TECHS/)
- [Requirements for Chinese Text Layout](https://www.w3.org/TR/clreq/), [Requirements for Japanese Text Layout](https://www.w3.org/TR/jlreq/), [Requirements for Hangul Text Layout and Typography](https://www.w3.org/TR/klreq/), [Ethiopic Layout Requirements](https://www.w3.org/TR/elreq/) (уже в группе типографики, но также связаны с интернационализацией)

### 20. Обзорные документы (Snapshots)
- CSS Snapshot [2021](https://www.w3.org/TR/css-2021/), [2022](https://www.w3.org/TR/css-2022/), [2023](https://www.w3.org/TR/css-2023/), [2024](https://www.w3.org/TR/css-2024/), [2025](https://www.w3.org/TR/css-2025/), [2026](https://www.w3.org/TR/css-2026/)
- [Cascading Style Sheets Level 2 Revision 1 (CSS 2.1) Specification](https://www.w3.org/TR/CSS2/)
- [Cascading Style Sheets Level 2 Revision 2 (CSS 2.2) Specification](https://www.w3.org/TR/CSS22/)

### 21. Прочие (Miscellaneous)
- CSS Containment Module [Level 1](https://www.w3.org/TR/css-contain-1/), [Level 2](https://www.w3.org/TR/css-contain-2/), [Level 3](https://www.w3.org/TR/css-contain-3/)
- [CSS Round Display Level 1](https://www.w3.org/TR/css-round-display-1/)
- [Priorities for CSS from the Digital Publishing Interest Group](https://www.w3.org/TR/dpub-css-priorities/)

---

Эта группировка охватывает все спецификации из предоставленного списка. Некоторые модули (например, CSS Writing Modes) могут влиять сразу на несколько аспектов (макет и типографику), поэтому они помещены в наиболее релевантную группу, но их многопрофильность стоит учитывать.
